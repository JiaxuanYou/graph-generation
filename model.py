...
class GRU_Graph_Class(nn.Module):
    ...
    def forward(self, input_raw, pack=False, input_len=None):
        ...
        output_classification = self.classification(graph_embedd)

        return output_raw, output_classification
...
class GRU_plain(nn.Module):
    ...
    def forward(self, input_raw, pack=False, input_len=None):
        ...
        if self.has_output:
            output_raw = self.output(output_raw)
        # return hidden state at each time step
        # If we want to do graph classification / have a graph embedding
        # based on the last hidden state of the graph level RNN we need to 
        # access self.hidden! Note we do not need to explicitally return it.
        return output_raw
