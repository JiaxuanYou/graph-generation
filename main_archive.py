...
##########
# archived main function, including the StructureRNN+CNN+attention model, GAN model, etc
# Dec 2, 2017
...
class Args():
    def __init__(self):
        ...
        self.max_prev_node = 50 # default value
...
def train_vae_epoch(epoch, args, lstm, output_vae, dataset,
                    ...
                    scheduler_lstm, scheduler_vae, train=True, gumbel=0, temperature=1):
    lstm.zero_grad()
    ...
    if train:
        ...
        # if backprop through the start
            ...
            if gumbel>np.random.rand(): # prob of using gumbel
                y_pred = Variable(torch.zeros(y.size(0), y.size(1), y.size(2))).cuda(CUDA) # normalized score
                z_mu = Variable(torch.zeros(y.size(0), y.size(1), args.embedding_size)).cuda(CUDA) # normalized score
                z_lsgms = Variable(torch.zeros(y.size(0), y.size(1), args.embedding_size)).cuda(CUDA) # normalized score

                x_step = x[:, 0:1, :]
                for i in range(x.size(1)):
                    h = lstm(x_step)
                    y_pred_step,z_mu_step,z_lsgms_step = output_vae(h,gumbel=True,temperature=temperature)
                    y_pred[:, i:i + 1, :] = y_pred_step # write down score
    ...
    if train:
        log_value('bce_loss_' + args.note + '_' + args.graph_type + '_' + str(args.num_layers) + '_' + str(
            args.bptt) + '_' + str(args.bptt_len) + '_' + str(args.gumbel), loss_bce.data[0],
                  epoch)
        log_value('kl_loss_' + args.note + '_' + args.graph_type + '_' + str(args.num_layers) + '_' + str(
            args.bptt) + '_' + str(args.bptt_len) + '_' + str(args.gumbel), loss_kl.data[0],
                  epoch)
        log_value('z_mu_mean_' + args.note + '_' + args.graph_type + '_' + str(args.num_layers) + '_' + str(
            args.bptt) + '_' + str(args.bptt_len) + '_' + str(args.gumbel), z_mu_mean,
                  epoch)
        log_value('z_mu_min_' + args.note + '_' + args.graph_type + '_' + str(args.num_layers) + '_' + str(
            args.bptt) + '_' + str(args.bptt_len) + '_' + str(args.gumbel), z_mu_min,
                  epoch)
        log_value('z_mu_max_' + args.note + '_' + args.graph_type + '_' + str(args.num_layers) + '_' + str(
            args.bptt) + '_' + str(args.bptt_len) + '_' + str(args.gumbel), z_mu_max,
                  epoch)
        log_value('z_sgm_mean_' + args.note + '_' + args.graph_type + '_' + str(args.num_layers) + '_' + str(
            args.bptt) + '_' + str(args.bptt_len) + '_' + str(args.gumbel), z_sgm_mean,
                  epoch)
        log_value('z_sgm_min_' + args.note + '_' + args.graph_type + '_' + str(args.num_layers) + '_' + str(
            args.bptt) + '_' + str(args.bptt_len) + '_' + str(args.gumbel), z_sgm_min,
                  epoch)
        log_value('z_sgm_max_' + args.note + '_' + args.graph_type + '_' + str(args.num_layers) + '_' + str(
            args.bptt) + '_' + str(args.bptt_len) + '_' + str(args.gumbel), z_sgm_max,
                  epoch)
...
def train_vae(args, dataset_train, lstm, output_vae):
    ...
    epoch = 0
    while epoch<=args.epochs:
        temperature = np.exp((-5e-4)*epoch)
        if epoch<args.epochs_gumbel:
            gumbel = 0
        else:
            gumbel = 1-np.exp((-1e-4)*(epoch-args.epochs_gumbel))
        # train
        train_vae_epoch(epoch, args, lstm, output_vae, dataset_train,
                        optimizer_lstm, optimizer_vae,
                        scheduler_lstm, scheduler_vae, train=True,gumbel=gumbel,temperature=temperature)
        # test
        if epoch % args.epochs_test == 0:
            train_vae_epoch(epoch, args, lstm, output_vae, dataset_train,
                            optimizer_lstm, optimizer_vae,
...
if __name__ == '__main__':
    ...
    print('max previous node', args.max_prev_node)
    ################## the GraphRNN model #####################
    ### 'Graph_sequence_sampler_rnn' is used for baseline model
    sampler = Graph_sequence_sampler_truncate(graphs, max_node_num=max_num_nodes, batch_size=args.batch_size, max_prev_node=args.max_prev_node)
    ...
    ### Graph RNN VAE model
    lstm = LSTM_plain(feature_size=args.max_prev_node, input_size=args.input_size,
                      hidden_size=args.hidden_size, batch_size=args.batch_size,
                      num_layers=args.num_layers).cuda(CUDA)
    ## if using vae
    output_vae = MLP_VAE_plain(h_size=args.hidden_size, embedding_size=args.embedding_size, y_size=args.max_prev_node).cuda(CUDA)
